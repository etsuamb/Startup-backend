...
const STARTUP_DOCUMENT_TYPES = [
	{
		key: "founder_id",
		label: "Founder or representative ID",
		accept: "application/pdf,.pdf,image/*",
	},
	{
		key: "business_registration_proof",
		label: "Business registration proof",
		accept: "application/pdf,.pdf,image/*",
	},
	{
		key: "support_affiliation_letter",
		label: "Support or affiliation letter",
		accept: "application/pdf,.pdf,image/*",
	},
	{
		key: "tin_certificate",
...
function isDocOfType(doc, typeKey) {
	...
	if (typeKey === "founder_id")
		return description.includes("founder") && description.includes("id");
	if (typeKey === "business_registration_proof")
		return (
			description.includes("business registration") ||
			description.includes("registration proof")
		);
	if (typeKey === "support_affiliation_letter")
		return (
			description.includes("support") || description.includes("affiliation")
		);
	if (typeKey === "tin_certificate") return description.includes("tin");
	if (typeKey === "proof_of_address")
		return (
			description.includes("proof of address") ||
			description.includes("address")
		);
	if (typeKey === "logo")
		return description.includes("logo") || fileName.includes("logo");
	return false;
}

function getDocumentTypeLabel(doc) {
	const found = STARTUP_DOCUMENT_TYPES.find((type) =>
		isDocOfType(doc, type.key),
	);
...
function IconFile() {
	return (
		<svg
			className="w-8 h-8 text-[#0f3d32] shrink-0"
			fill="none"
			stroke="currentColor"
			viewBox="0 0 24 24"
		>
...
export default function StartupSettingsPage() {
	...
	const otherDocuments = useMemo(
		() =>
			documents.filter(
				(doc) =>
					!STARTUP_DOCUMENT_TYPES.some((type) => isDocOfType(doc, type.key)),
			),
		[documents],
	);
	...
	const loadProfile = useCallback(async () => {
		...
		try {
			...
			if (!profile) {
				showToast(
					"No startup profile found. Complete registration first.",
					"error",
				);
				return;
			}
	...
	async function handleSaveAccountInfo(event) {
		...
		try {
			...
			showToast(
				err.message || "Unable to update account information.",
				"error",
			);
		} finally {
			setSaving(false);
	...
	async function handleSaveProfile(event) {
		...
		formData.append(
			"founder_full_name",
			founderFullName.trim() || `${firstName.trim()} ${lastName.trim()}`.trim(),
		);
		...
		formData.append(
			"location",
			location.trim() ||
				[region.trim(), city.trim()].filter(Boolean).join(", "),
		);
		...
		if (fundingNeeded.trim())
			formData.append("funding_needed", fundingNeeded.trim());
			...
			<form onSubmit={handleSaveAccountInfo} className="space-y-6">
				<SectionCard>
					<SectionHeader
						...
						description="Your login credentials and contact information."
					/>

					{/* Status badge */}
					<div className="mb-6 flex items-center gap-3 rounded-xl bg-gray-50 border border-gray-100 px-4 py-3">
						<div
							className={`w-2.5 h-2.5 rounded-full shrink-0 ${adminStatus === "approved" ? "bg-emerald-500" : "bg-amber-500"}`}
						/>
						<div>
						...
						>
							...
							{!emailVerified ? (
							  ...
							  originalEmail.trim().toLowerCase() ? (
								<p className="mt-2 text-xs font-semibold text-amber-700">
									Saving will send a new verification link and mark the account
					...
					<div className="mt-5">
						<FormField
							...
							hint="As displayed on your public startup profile."
						>
							<input
			...
			<form onSubmit={handleSaveProfile} className="space-y-6">
				<SectionCard>
					<SectionHeader
						...
						description="Information that investors and mentors see on your profile."
					/>
					...
					<div className="space-y-5">
						...
						<FormField
							label="Tagline"
							hint="A short, punchy description of what your startup does."
						>
							<input
								value={tagline}
								onChange={(e) => setTagline(e.target.value)}
								className={inputClass}
								placeholder="e.g., Making fintech accessible for everyone"
							/>
						</FormField>
				...
				<SectionCard>
					<SectionHeader
						title="Team & Location"
						description="Help investors understand your team and where you operate."
					/>
					...
					<div className="flex items-center justify-between p-4 rounded-xl border border-emerald-200 bg-emerald-50/40 hover:bg-emerald-50 transition">
						...
						>
							...
							<div className="min-w-0">
								...
								<p className="text-[11px] text-[#0f3d32] font-semibold mt-0.5">
									{getDocumentTypeLabel(currentDoc)}
								</p>
								<p className="text-xs text-gray-500 mt-0.5">
									Uploaded{" "}
									{new Date(currentDoc.created_at).toLocaleDateString()}
								</p>
							</div>
						</button>
						<button
					...
					<div className="space-y-2">
						<div className="rounded-xl border-2 border-dashed border-gray-200 bg-gray-50/50 p-5 text-center hover:border-[#0f3d32]/30 transition">
							...
							<p className="mt-2 text-xs text-gray-400">
								Upload a clearer or updated file to replace the current one.
							</p>
						</div>
						{currentDoc && (
					...
					<div className="space-y-6">
						{registrationDocuments.map((type, index) => (
							<div key={type.key}>
								...
								{index !== registrationDocuments.length - 1 && (
				...
				{otherDocuments.length > 0 && (
					<SectionCard>
						<SectionHeader
							...
							description={`${otherDocuments.length} additional document${otherDocuments.length !== 1 ? "s" : ""} uploaded.`}
						/>
						<ul className="space-y-2">
							{otherDocuments.map((doc) => (
								<li
									key={doc.document_id}
									className="rounded-xl bg-gray-50 border border-gray-100 hover:border-[#0f3d32]/20 hover:bg-[#f0faf7] transition"
								>
									<ViewableFileTrigger
									...
									>
										<span className="flex justify-between gap-3 items-center w-full px-4 py-3">
											...
											<span className="text-xs font-bold text-[#0f3d32] shrink-0">
												View →
											</span>
										</span>
									</ViewableFileTrigger>
			...
			<div className="space-y-6">
				...
				<SectionCard>
					<SectionHeader
						...
						description="Update your login password. Use a strong combination of letters, numbers, and symbols."
					/>
								...
								<div className="w-44 h-44 rounded-2xl bg-white border-2 border-gray-200 flex items-center justify-center">
									<div className="grid grid-cols-8 grid-rows-8 gap-0.5 w-32 h-32">
										{Array.from({ length: 64 }, (_, i) => (
											<div
												key={i}
												className={`w-full aspect-square rounded-[1px] ${Math.random() > 0.5 ? "bg-gray-900" : "bg-white"}`}
											/>
										))}
									</div>
								</div>
				...
				<SectionCard className="border-red-100">
					...
					<div className="rounded-xl bg-amber-50 border border-amber-200 p-4 mb-4">
						<p className="text-xs text-amber-800">
							<strong>What happens:</strong> Your profile, projects, and offers
							will be hidden from investors and mentors. Active mentorship
							sessions will be paused. No data will be deleted.
						</p>
					</div>
					<button
						type="button"
				...
				<SectionCard className="border-red-200 bg-red-50/30">
					...
					<div className="rounded-xl bg-red-50 border border-red-200 p-4 mb-5">
						<p className="text-xs text-red-800">
							<strong>⚠ Warning:</strong> All your data including profile,
							projects, documents, offers, chat history, and mentorship records
		...
		<div className="min-h-screen bg-[#f6f8f9] font-sans text-gray-900 flex">
			...
			<main className="flex-grow overflow-y-auto">
				<div className="w-full max-w-[1200px] mx-auto px-4 sm:px-8 py-8">
					{/* Page Header */}
					<div className="mb-8">
						<p className="text-[10px] font-bold uppercase tracking-widest text-[#0f3d32]">
							Startup · Settings
						</p>
						...
						<p className="mt-1.5 text-sm text-gray-500">
							Manage your account, startup profile, security, and notification
							preferences.
						</p>
					</div>
