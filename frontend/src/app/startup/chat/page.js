...
function normalizeInvestorConversation(row) {
  const contactName =
    `${row.investor_first_name || ""} ${row.investor_last_name || ""}`.trim() || "Investor";
  const company = row.investor_type || row.investor_email || row.investor_company || "Investor";
  return {
...
export default function StartupInvestorChatPage() {
  return (
    <StartupChatView
      ...
      messageId={(m) => m.investor_chat_message_id}
