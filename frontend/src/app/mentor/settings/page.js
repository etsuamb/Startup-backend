...
function SectionCard({ children, className = "" }) {
	return (
		<div
			className={`rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-sm ${className}`}
		>
			{children}
		</div>
	);
				...
				<div className="mt-4">
					...
					<div className="flex flex-wrap gap-1.5">
						{missing.slice(0, 6).map((f) => (
							<span
								key={f.label}
								className="text-[11px] font-medium text-gray-500 bg-gray-50 border border-gray-100 px-2 py-1 rounded-lg"
							>
								{f.label}
							</span>
						))}
...
export default function MentorSettingsPage() {
		...
		fetchMentorProfile()
			.then((data) => {
				...
				if (p.session_duration)
					setSessionDuration(fieldValue(p.session_duration));
				...
				if (typeof p.accepting_mentees === "boolean")
					setAcceptingMentees(p.accepting_mentees);
				else if (typeof availability.accepting_mentees === "boolean")
					setAcceptingMentees(availability.accepting_mentees);
				else if (p.availability_preference)
					setAcceptingMentees(
						!/not|unavailable|closed/i.test(p.availability_preference),
					);
			})
			.catch(async (err) => {
				...
				try {
					...
					showToast(
						accountErr.message || "Unable to load account information.",
						"error",
					);
				}
			})
			...
			<div className="space-y-6">
					...
					<div className="flex items-center gap-6">
						...
						<div>
							...
							<div className="mt-3 flex items-center gap-2">
								...
								>
									<span
										className={`w-1.5 h-1.5 rounded-full ${acceptingMentees ? "bg-emerald-500" : "bg-gray-400"}`}
									/>
					...
					<div className="space-y-5">
						...
						<div className="grid gap-5 sm:grid-cols-2">
							<FormField
								...
								hint={
									emailVerified
										? "Changing this email requires verification again."
										: "Use a real email you can access, then verify it from your inbox."
								}
							>
								<input
									type="email"
								...
								{!emailVerified ? (
									<p className="mt-2 text-xs font-semibold text-amber-700">
										This account cannot be approved until this email is
										verified.
									</p>
								) : email.trim().toLowerCase() !==
								  originalEmail.trim().toLowerCase() ? (
									<p className="mt-2 text-xs font-semibold text-amber-700">
										Saving will send a new verification link and mark the
										account unverified until confirmed.
									</p>
								) : null}
							</FormField>
				...
				<SectionCard>
					<SectionHeader
						...
						description="Help startups understand your background and expertise."
					/>
					<div className="space-y-5">
						<FormField
							label="Professional headline"
							hint="Shown prominently on your mentor profile card."
						>
							<input
								id="mentor-headline"
								value={headline}
						...
						<FormField
							label="Bio"
							hint="Write a compelling mentor bio. 150–300 words is ideal."
						>
							<textarea
								id="mentor-bio"
								value={bio}
			...
			<div className="space-y-6">
				<SectionCard>
					<SectionHeader
						...
						description="Select all the areas where you can guide startups. This helps match you with the right mentees."
					/>
					...
					{expertiseAreas.length > 0 && (
						<p className="mt-4 text-xs text-emerald-700 font-semibold bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-2.5">
							✓ {expertiseAreas.length} area
							{expertiseAreas.length !== 1 ? "s" : ""} selected
						</p>
					)}
				</SectionCard>
				...
				<SectionCard>
					<SectionHeader
						...
						description="Your primary industry focus helps startups find the right mentor."
					/>
					<div className="grid gap-5 sm:grid-cols-2">
						<SelectField
						...
						/>
						<FormField
							label="Mentorship focus statement"
							hint="A one-liner about your unique mentorship style."
						>
							<input
								id="mentor-focus"
				...
				<SectionCard>
					<SectionHeader
						title="Mentorship Capacity"
						description="Manage your workload and pricing."
					/>
					<div className="grid gap-5 sm:grid-cols-2">
						<FormField
							label="Max active startups"
							hint="Maximum startups you can mentor at once."
						>
							<input
						...
						<FormField
							label="Session rate (ETB)"
							hint="Leave blank if you mentor for free."
						>
							<input
			...
			<div className="space-y-6">
					...
					<div className="flex items-center justify-between gap-4">
						<div>
							...
							<p className="text-sm text-gray-500 mt-1">
								When turned off, you won't appear in search results and startups
								cannot send you new requests.
							</p>
						</div>
						<ToggleSwitch
							enabled={acceptingMentees}
							onToggle={() => setAcceptingMentees((v) => !v)}
						/>
					</div>
					{!acceptingMentees && (
				...
				<SectionCard>
					<SectionHeader
						...
						description="Set your working hours so startups know when to book sessions."
					/>
					<div className="space-y-5">
						<SelectField
							label="Timezone"
				...
				<SectionCard>
					<SectionHeader
						...
						description="Select the days of the week you are available for sessions."
					/>
		...
		const emailNotifs = [
			{
				...
				description:
					"Receive email when a startup sends you a mentorship request.",
			},
			{
				key: "email_session_remind",
				title: "Session Reminders",
				description: "Get reminded 1 hour before a scheduled session.",
			},
			{
				key: "email_message",
				title: "New Messages",
				description: "Email alert when you receive a new chat message.",
			},
			{
				key: "email_report_due",
				title: "Report Due Alerts",
				description: "Be notified when a progress report is due.",
			},
		];
		const pushNotifs = [
			{
				key: "push_new_request",
				title: "Push: New Request",
				description: "Instant push notification for new mentorship requests.",
			},
			{
				key: "push_session_remind",
				title: "Push: Session Reminder",
				description: "Push notification 15 minutes before a session starts.",
			},
			{
				key: "push_message",
				title: "Push: Messages",
				description: "Real-time notification for new messages.",
			},
			{
				key: "push_report_due",
				title: "Push: Report Due",
				description: "Push alert when a report submission deadline is near.",
			},
		];
		const digestNotifs = [
			{
				key: "weekly_digest",
				title: "Weekly Digest",
				description: "A weekly summary of your mentorship activity.",
			},
			{
				key: "monthly_summary",
				title: "Monthly Summary",
				description: "Monthly analytics and impact report to your email.",
			},
		];
			...
			<div className="space-y-6">
				<SectionCard>
					<SectionHeader
						...
						description="Control which emails you receive from StartupConnect."
					/>
				...
				<SectionCard>
					<SectionHeader
						...
						description="Manage browser and mobile push alerts."
					/>
					<div className="divide-y divide-gray-50">
				...
				<SectionCard>
					<SectionHeader
						...
						description="Periodic summaries of your mentorship activity."
					/>
					<div className="divide-y divide-gray-50">
	...
	function renderSecurityTab() {
		const passwordStrength = () => {
			...
			if (score === 2)
				return { label: "Fair", color: "bg-amber-500", w: "w-2/4" };
			if (score === 3)
				return { label: "Good", color: "bg-blue-500", w: "w-3/4" };
			...
			<div className="space-y-6">
							...
							{strength && (
								<div className="mt-2">
									<div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
										<div
											className={`h-full rounded-full transition-all duration-500 ${strength.color} ${strength.w}`}
										/>
									</div>
									<p
					...
					{twoFAStep === "idle" && !twoFAEnabled && (
						<div className="rounded-xl bg-gray-50 border border-gray-100 p-6 text-center">
							...
							<p className="text-xs text-gray-500 mb-5 max-w-sm mx-auto">
								...
								Download Google Authenticator or Authy to get started.
							</p>
							<button
								type="button"
								id="enable-2fa-btn"
						...
						<div className="space-y-5">
								...
								<div className="w-44 h-44 rounded-2xl bg-white border-2 border-gray-200 flex items-center justify-center p-3">
									<div className="grid grid-cols-8 grid-rows-8 gap-0.5 w-full h-full">
										{Array.from({ length: 64 }, (_, i) => (
											<div
												key={i}
												className={`w-full aspect-square rounded-[1px] ${[0, 8, 16, 7, 15, 23, 56, 57, 58, 59, 60, 61, 62, 63, 48, 40, 32, 24].includes(i) || Math.random() > 0.6 ? "bg-gray-900" : "bg-white"}`}
											/>
										))}
									</div>
								</div>
							...
							>
								...
								<div className="flex gap-3">
									...
									<button
										type="submit"
										className="flex-1 rounded-xl bg-[#0f3d32] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#061e16] transition"
									>
										Verify & Enable
									</button>
								</div>
				...
				<SectionCard>
					<SectionHeader
						...
						description="Devices where your account is currently logged in."
					/>
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-xs text-gray-500">
										Current session • Addis Ababa, ET
									</p>
								</div>
							</div>
					...
					<div className="flex items-start gap-4">
						...
						<div className="flex-1 min-w-0">
							<h3 className="text-base font-bold text-gray-900 mb-1">
								Pause Mentorship Activity
							</h3>
							<p className="text-sm text-gray-500 mb-4">
								Temporarily pause your mentor account. You won't appear in
								search results, and no new requests can be sent to you. Your
								existing mentorships remain active.
							</p>
							<button
								type="button"
								id="pause-account-btn"
								onClick={() =>
									showToast("Your mentor account has been paused.", "error")
								}
				...
				<SectionCard>
					<div className="flex items-start gap-4">
						...
						<div className="flex-1 min-w-0">
							...
							<p className="text-sm text-gray-500 mb-4">
								Download all your mentor profile data, session history, and
								mentorship records as a JSON file.
							</p>
							<button
								type="button"
								id="export-data-btn"
					...
					<div className="flex items-start gap-4">
						...
						<div className="flex-1 min-w-0">
							...
							<p className="text-sm text-gray-600 mb-5">
								This permanently deletes your mentor account, profile, all
								session history, reports, and mentorship data.
	...
	return (
		<div className="min-h-full bg-[#f8fafc] text-[#061f1a]">
			...
			<header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-gray-100 bg-white px-5 sm:px-8 shadow-sm">
				<div>
					<h1 className="text-lg font-black text-[#052b23] tracking-tight">
						Settings
					</h1>
				...
				<div className="grid grid-cols-1 gap-8 lg:grid-cols-[240px_1fr]">
					...
					<aside className="lg:sticky lg:top-[88px] lg:self-start">
						...
						<div className="mt-4 rounded-xl border-l-2 border-[#0f3d32] bg-[#f0faf7] p-4">
							<p className="text-[10px] font-black text-[#0f3d32] uppercase tracking-widest mb-1">
								Mentor Tip
							</p>
							<p className="text-xs text-[#0a3026] leading-relaxed">
								Mentors with complete profiles receive <strong>3x more</strong>{" "}
								mentorship requests.
							</p>
						</div>
					</aside>
