...
export default function MentorDashboard() {
	...
	return (
		<div className="min-h-full bg-[#f6f8f7] text-[#061f1a]">
			...
			<main className="mx-auto grid w-full max-w-[1240px] grid-cols-1 gap-7 px-5 py-7 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8">
				<section className="min-w-0">
					{error ? (
						<div className="mb-6 rounded-xl border border-red-200 bg-red-50 px-5 py-4 text-sm font-medium text-red-700">{error}</div>
					) : null}

					<div className="mb-7 rounded-2xl border border-emerald-100/80 bg-gradient-to-r from-white to-[#eef8f4] p-6 shadow-sm lg:flex lg:items-end lg:justify-between">
						<div>
							<p className="mb-2 text-[10px] font-black uppercase tracking-[0.22em] text-[#168267]">Mentor overview</p>
						...
						<div className="mt-5 flex flex-wrap gap-2 lg:mt-0 lg:justify-end">
							{[
								...
								<div key={label} className="inline-flex h-8 items-center gap-2 rounded-full border border-emerald-100 bg-white/80 px-3 text-[11px] font-black shadow-sm">
					...
					<div className="mb-7 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
						{statCards.map((card, index) => (
							<div
								key={card.label}
								className={`relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${index === 0 ? "ring-1 ring-inset ring-emerald-100" : ""}`}
							>
					...
					<div className="mb-7 grid grid-cols-1 gap-4 md:grid-cols-2">
						{loading ? (
							<div className="rounded-2xl border border-slate-200/80 bg-white p-5 text-sm text-gray-500 shadow-sm">Loading sessions...</div>
						) : upcomingSessions.length === 0 ? (
							<div className="rounded-2xl border border-slate-200/80 bg-white p-5 text-sm text-gray-500 shadow-sm">No upcoming sessions.</div>
						) : (
							upcomingSessions.slice(0, 2).map((session) => (
								<div key={session.mentorship_session_id || session.session_id} className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition hover:shadow-md">
									...
									<div className="flex gap-2">
										<Link href="/mentor/sessions" className="flex-1 rounded-xl bg-[#0b4a3c] px-4 py-3 text-center text-xs font-black text-white transition hover:bg-[#07382d]">
											Join Session
										</Link>
					...
					<section>
						...
						<div className="rounded-2xl border border-emerald-100 bg-white p-5 text-[#052b23] shadow-sm">
							{progressStartups.length === 0 ? (
								<p className="text-sm text-slate-500">No active cohort progress yet.</p>
							) : (
										...
										return (
											<div key={startup.startup_id || startup.id || startup.startup_name || index}>
												...
												<div className="h-1.5 rounded-full bg-slate-100">
													<div className="h-1.5 rounded-full bg-[#168267]" style={{ width: `${progress}%` }} />
												</div>
											</div>
										);
									})}
				...
				<aside className="space-y-6">
					<section>
						...
						<div className="grid grid-cols-2 gap-3">
							{[
								...
								<Link key={label} href={href} className="flex min-h-[92px] flex-col items-center justify-center rounded-2xl border border-slate-200/80 bg-white p-4 text-center text-xs font-black text-gray-950 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-md">
									<Icon path={path} className="mb-3 h-5 w-5 text-[#168267]" />
									{label}
								</Link>
							))}
						</div>
					...
					<section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
						...
						<Link href="/mentor/messages" className="mt-5 flex h-9 items-center justify-center rounded-xl border border-gray-200 text-xs font-black text-gray-900 transition hover:border-emerald-200 hover:bg-emerald-50/50 hover:text-[#0b4a3c]">
							Go to Messages
						</Link>
					</section>

					<section className="rounded-2xl border border-emerald-100 bg-[#eef8f4] p-5">
