...
export default function InvestorReportsPage() {
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      ...
      setError("");
      try {
        const [profileRes, portfolioRes, offersRes, startupsRes, recommendationsRes] = await Promise.all([
          getInvestorProfile(),
          getInvestorPortfolio().catch(() => null),
          getInvestorFundingOffers().catch(() => ({ funding_offers: [] })),
          getInvestorStartups({ limit: 12 }).catch(() => ({ startups: [] })),
          getInvestorRecommendations({ limit: 8 }).catch(() => ({ recommendations: [] })),
        ]);
        if (!alive) return;
        setProfile(profileRes.investor || null);
        setPortfolio(portfolioRes || null);
        setOffers(offersRes.funding_offers || []);
        setStartups(startupsRes.startups || []);
        setRecommendations(recommendationsRes.recommendations || []);
      } catch (err) {
        if (alive) setError(err.message || "Unable to load investor reports.");
      } finally {
  ...
  const metrics = useMemo(() => {
    const pending = offers.filter((offer) => String(offer.status || "").toLowerCase() === "pending");
    const accepted = offers.filter((offer) => ["accepted", "approved"].includes(String(offer.status || "").toLowerCase()));
    const conversion = offers.length ? Math.round((accepted.length / offers.length) * 100) : 0;
    const committed = offers.reduce((sum, offer) => sum + Number(offer.amount || offer.investment_amount || 0), 0);
    const readiness = Math.min(100, 35 + (conversion ? 20 : 0) + (recommendations.length ? 20 : 0) + (Number(portfolio?.total_value || 0) ? 25 : 0));
    return {
      ...
      totalInvestments: Number(portfolio?.total_investments || accepted.length || 0),
    };
  }, [offers, portfolio, recommendations.length]);

  const reportText = useMemo(() => {
    return [
      "StartupConnect Investor Report",
      `Generated: ${new Date().toLocaleString()}`,
      "",
      `Investor: ${investorName(profile)}`,
      `Type: ${profile?.investor_type || "Not provided"}`,
      `Preferred Industry: ${profile?.preferred_industry || "Not provided"}`,
      `Portfolio Value: ${formatMoney(metrics.portfolioValue)}`,
      `Offer Conversion: ${metrics.conversion}%`,
      ...
      ...(recommendations.length
        ? recommendations.map((item) => {
            ...
            return `- ${startup.startup_name || startup.project_title || "Startup"}: ${item.reason || startup.industry || "Recommended"}`;
          })
        : ["- No current recommendations"]),
    ].join("\n");
  }, [metrics, profile, recommendations]);

  return (
    <div className="flex h-screen overflow-hidden bg-[#f4f7f6] font-sans text-gray-900">
      <Sidebar />
          ...
          <section className="min-w-0 space-y-5">
            <div className="relative overflow-hidden rounded-lg bg-[#073f32] p-8 text-white shadow-sm">
              ...
              <div className="relative max-w-xl">
                ...
                <h1 className="mt-5 text-3xl font-black tracking-tight">Generate an investor portfolio report</h1>
                <p className="mt-3 text-sm leading-6 text-emerald-50/80">
                  Aggregate portfolio value, offer conversion, startup pipeline, and recommendations into a concise report.
                </p>
                <button
                  type="button"
                  onClick={() => downloadReport(`investor-report-${Date.now()}.txt`, reportText)}
                  disabled={loading}
            ...
            <div className="grid grid-cols-2 rounded-lg bg-white p-1 shadow-sm">
              {[["readiness", "Portfolio / Pipeline"], ["operations", "Investor Readiness / Operations"]].map(([key, label]) => (
                <button
                  key={key}
                  type="button"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <MetricCard icon="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V7m0 9v1" label="Portfolio Value" value={loading ? "..." : formatMoney(metrics.portfolioValue)} note="+12%" />
              <MetricCard icon="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" label="Offer Conversion" value={loading ? "..." : `${metrics.conversion}%`} note="On track" tone="blue" />
              <MetricCard icon="M12 6v6l4 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z" label="Pending Offers" value={loading ? "..." : metrics.pending} note="3 pending" tone="gray" />
              <MetricCard icon="M9.813 15.904L9 18.75l-.813-2.846A4.5 4.5 0 005.096 12L2.25 12l2.846-.813A4.5 4.5 0 008.187 8.096L9 5.25l.813 2.846A4.5 4.5 0 0012.904 11.187L15.75 12l-2.846.813a4.5 4.5 0 00-3.091 3.091z" label="Recommendations" value={loading ? "..." : recommendations.length} note={`${startups.length} startups`} />
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <InsightItem title="Offer conversion is trending upward" body={`${metrics.conversion}% of visible offers are accepted or approved.`} />
                <InsightItem title="Recommendation pipeline is active" body={`${recommendations.length} AI-matched startup${recommendations.length === 1 ? "" : "s"} ready for review.`} />
              </div>
            </div>
          </section>
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <ChecklistItem label="Portfolio Metrics" checked />
                <ChecklistItem label="KPI Dashboards" checked />
                <ChecklistItem label="Startup Pipeline" checked={false} />
