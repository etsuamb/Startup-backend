...
export function PendingApprovalBanner({ className = "" }) {
  return (
    <div
    ...
    >
      ...
      <p className="mt-1 text-amber-800">
        Your account must be approved before you can send offers, create projects, message investors or mentors,
        view full profiles, or submit ratings.
      </p>
    </div>
  );
}

export function PendingApprovalBlock({ title = "Action unavailable" }) {
  return (
      ...
      <main className="flex-grow flex items-center justify-center p-8">
        <div className="max-w-lg w-full rounded-[32px] border border-gray-100 bg-white p-8 shadow-sm text-center">
          ...
          <p className="mt-3 text-sm text-gray-600">
            Your startup account is pending admin approval. You will be able to use this feature once an
